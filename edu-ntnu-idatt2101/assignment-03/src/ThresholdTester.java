...
import java.util.Random;
  ...
  public class ThresholdTester {
    ...
    public static void main(String[] args) {
      // Generate a single random array
      int arraySize = 10_000;
      int testSize = 10; // Number of times to run the sorting
      int[] testArray = randomIntArray(arraySize);

      int maxThreshold = 1_000; // Set the maximum threshold you want to test
      long minTime = Long.MAX_VALUE;
      int optimalThreshold = 0;

      for (int threshold = 1; threshold <= maxThreshold; threshold++) {
        THRESHOLD = threshold; // Set the global THRESHOLD variable
        long totalRunTime = 0;

        for (int j = 0; j < testSize; j++) {
          int[] array = testArray.clone(); // Clone the array to avoid changes
        ...
        long avgRunTime = totalRunTime / testSize;

        if (avgRunTime < minTime) {
          minTime = avgRunTime;
          optimalThreshold = threshold;
        }

        System.out.println("Threshold: " + threshold + ", Average Time: " + avgRunTime);
      }

      System.out.println("Optimal threshold is " + optimalThreshold);
    }
...
//    for(iStart = start, iEnd = end-1;;)
//    {
//      while(array[++iStart] < middleValue);
//      while(array[--iEnd] > middleValue);
//      if(iStart >= iEnd)
//        break;
//      swap(array, iStart, iEnd);
//    }
